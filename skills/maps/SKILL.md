...
Script path: `scripts/maps_client.py` inside this skill directory. The helper
script is not bundled with this port — fetch it from the upstream source
at `https://github.com/NousResearch/hermes-agent/tree/main/skills/productivity/maps/scripts`
or re-implement the commands below via direct HTTPS calls to Nominatim,
Overpass, and OSRM.
...
```bash
# By coordinates (from a location pin, for example)
...
`maps_url` field so the user gets a tap-to-open link. For "open
now?" questions, check the `hours` field; if missing or unclear, verify
with a web search since OSM hours are community-maintained and not always
current.
...
1. Extract lat/lon from the message
