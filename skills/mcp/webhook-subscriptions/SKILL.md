...
metadata:
  hermes:
    tags: [webhook, events, automation, integrations, notifications, push, mcp]
---

# Webhook Subscriptions

Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger agent runs by POSTing events to a URL.

> **Runtime note:** The `hermes gateway` / `hermes webhook` CLI, the `~/.hermes/`
> config paths, and the webhook adapter referenced below come from the upstream
> Hermes Agent runtime. This skill documents the proven patterns (HMAC
> validation, prompt templating, subscription persistence, direct delivery). If
> you are running Claude Code without the Hermes runtime, map these commands
> onto your own webhook gateway (for example an MCP webhook server, an n8n
> trigger, a Cloudflare Worker, or a local FastAPI service) while keeping the
> same payload shape, security model, and event filtering rules.
