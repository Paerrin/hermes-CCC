...
metadata:
  hermes:
    ...
    upstream_authors: "Hermes Agent + Teknium"
---
...
qmd exposes an MCP server that provides search tools directly to Claude Code
via its native MCP client. This is the preferred integration — once
...
When MCP is not configured, use qmd directly via the `Bash` tool:

```
Bash(command="qmd query 'what was decided about the API redesign' --json", timeout=30)
```

For setup and management tasks, always use the shell:

```
Bash(command="qmd collection add ~/Documents/notes --name notes")
Bash(command="qmd context add qmd://notes 'Personal research notes and ideas'")
Bash(command="qmd embed")
Bash(command="qmd status")
```
