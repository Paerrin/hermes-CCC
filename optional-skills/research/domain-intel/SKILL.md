...
metadata:
  hermes:
    tags: [osint, domain, dns, ssl, whois, subdomains, reconnaissance]
    category: research
    upstream_author: "@FurkanL0"
---
...
- User asks about a domain's infrastructure (DNS, SSL, subdomains, WHOIS)
- User wants to check if a domain is available for registration
- User needs a bulk scan across several domains at once
- User wants a passive baseline before any active reconnaissance
- Keywords: "subdomains", "SSL expiry", "who owns", "is this domain available", "DNS records"

Do NOT use this skill for port scanning, vulnerability testing, or any active probing — this is passive only.
...
Every command emits JSON to stdout. Typical shapes:

- `subdomains`: `{"domain": "...", "subdomains": [...], "count": N, "source": "crt.sh"}`
- `ssl`: `{"domain": "...", "issuer": "...", "valid_until": "ISO-8601", "days_remaining": N, "sans": [...]}`
- `whois`: `{"domain": "...", "registrar": "...", "created": "ISO-8601", "expires": "ISO-8601", "nameservers": [...]}`
- `dns`: `{"A": [...], "AAAA": [...], "MX": [...], "NS": [...], "TXT": [...], "CNAME": [...]}`
- `available`: `{"domain": "...", "likely_available": bool, "confidence": "high|medium|low", "signals": {...}}`
- `bulk`: array of per-domain objects shaped by the checks requested.

## Failure Modes

- TCP/43 blocked by a corporate firewall — WHOIS times out. Fall back to `available` or `dns` which work over HTTPS.
- crt.sh returns 502/timeout for very popular domains — retry with backoff, or pass `--page-size` if supported to paginate.
- SSL handshake refused — either the domain does not serve TLS on 443, or the hostname does not match the cert. Inspect `dns` first to confirm the A/AAAA record resolves.
- Redacted WHOIS (GDPR) — name, email, phone will be stripped. Registrar, dates, and nameservers remain useful.
- False positive on `available` — some parked domains have SSL and DNS but no WHOIS record. Always corroborate with a registrar search before claiming availability.
...
*Contributed upstream by [@FurkanL0](https://github.com/FurkanL0).*
