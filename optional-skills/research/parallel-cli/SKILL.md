...
This is an optional third-party workflow, not a core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Claude Code's built-in `WebSearch` / `WebFetch`, so do not prefer it by default for ordinary lookups.
...
Prefer Claude Code's built-in `WebSearch` / `WebFetch` for quick one-off lookups when Parallel is not specifically requested.
...
If auth requires browser interaction, run with a PTY.
...
7. Prefer Claude Code's built-in web tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
- Do not silently choose Parallel when the built-in Claude Code web tools are already sufficient.
